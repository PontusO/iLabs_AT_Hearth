 ...
 *   No version byte at all. Every legal v1 composition has 0..28 endpoints,
...
#define MT_COMP_MAX_ENDPOINTS 28

/* v3 blob discriminator: see the encoding note above. A v1 blob's first
 * byte is a count (0..28), so 0xFF is free to mark v2/v3 unambiguously. */
...
/* Maximum encoded blob size: sentinel + version + u16 count, then up to 28
